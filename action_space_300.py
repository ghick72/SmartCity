...
if __name__ == "__main__":
    # SmartCityEnvironment 인스턴스 생성
    env = SmartCityEnvironment(render_speed=0.1)
    
    # 테스트를 위한 임의의 액션 선택 및 실행
    # 액션은 (건물 유형, x 위치, y 위치)로 구성됩니다.
    test_actions = [
        (0, 1, 1),  # 주거공간
        (1, 2, 2),  # 상업공간
        (2, 3, 3),  # 산업공간
    ...
    for action in test_actions:
        building_type, x, y = action
        action_index = env.actions.index(action)  # 액션 인덱스 조회
        env.place_building(building_type, x, y)  # 건물 배치

    # step 함수 호출 및 결과 출력
    state, reward, done, _ = env.step(action_index)  # 마지막 액션 인덱스 사용
